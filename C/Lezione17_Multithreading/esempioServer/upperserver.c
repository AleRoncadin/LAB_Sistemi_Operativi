...
#include <sys/types.h>
#include <sys/wait.h>
#include <pthread.h> // Aggiunta della libreria pthread.h

#include "common.h"

void upperlines(int in, int out);
void handle_sigchld(int);
void *handle_connection(void *arg); // Prototipo della funzione di gestione della connessione
...
int main (int argc, char **argv)
{
  ...
  while (1)
  {
    ...
    pthread_detach(tid); // Permette al thread di liberare automaticamen
  }
}

void *handle_connection(void *arg)
{
  int fd = *((int *)arg); // Ottiene il descrittore del file dalla connessione accettata

  fprintf(stderr, "Aperta connessione (Thread %ld).\n", pthread_self());
